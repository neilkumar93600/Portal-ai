---
name: master
description: "Universal task router and orchestrator. Use this skill when the user gives a broad, multi-domain, or ambiguous task that might span UI design, SEO, marketing, conversion, email, pricing, referral programs, frontend development, backend APIs, databases, Node.js, design systems, or video. Also use when the user says 'help me with my project,' 'I need everything done,' 'do whatever it takes,' 'full stack help,' 'improve my site,' 'launch my product,' 'build an API,' 'set up my backend,' or any other broad request. This skill reads the task, identifies which combination of skills is needed, and runs them in the right order — potentially multiple skills in parallel or sequence. Available skills it can orchestrate: frontend-design, ui-ux-pro-max, shadcn, shadcn-ui, tailwind-design-system, web-design-guidelines, backend-development, backend-dev-guidelines, nodejs-backend-patterns, seo-audit, ai-seo, audit-website, page-cro, popup-cro, email-sequence, pricing-strategy, referral-program, remotion-best-practices, skill-creator."
---
...
| Task | Skill |
|------|-------|
| Build REST APIs, GraphQL, gRPC; design auth (OAuth, JWT); set up databases (PostgreSQL, MongoDB, Redis); Docker, Kubernetes, CI/CD, microservices, security (OWASP), performance optimization | `backend-development` |
| Express middleware, controllers, services, repositories, Prisma database access, production-grade service architecture | `backend-dev-guidelines` |
| Node.js backend with Express/Fastify, middleware patterns, error handling, REST/GraphQL APIs, microservices | `nodejs-backend-patterns` |
...
1. `backend-development` — API design, auth, database schema, security
2. `nodejs-backend-patterns` — Express/Fastify middleware, error handling, REST structure
3. `shadcn-ui` + `tailwind-design-system` — frontend component setup
4. `frontend-design` — UI pages
5. `pricing-strategy` — packaging and monetization

### "Set up my Node.js API with Express and Prisma"
1. `nodejs-backend-patterns` — server structure, middleware, routing
2. `backend-dev-guidelines` — controllers, services, repositories, Prisma access patterns
